...
bool LoadIPsFromHttp(const CService& addrConnect, const char* pszGet, CNetAddr *pSeeds, int *pCount)
{
    SOCKET hSocket;
    if (!ConnectSocket(addrConnect, hSocket))
        return error("LoadIPsFromHttp() : connection to %s failed", addrConnect.ToString().c_str());
    ...
    closesocket(hSocket);
    return error("LoadIPsFromHttp() : connection closed");
}
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (!fTestNet)
    {
        printf("Loading addresses from apecoin.org\n");

		CService addrConnect;

        CService addrIP("seed.apecoin.org", 80, true);
        if (addrIP.IsValid())
            addrConnect = addrIP;


        char *pszGet = "GET / HTTP/1.1\r\n"
                    "Host: seed.apecoin.org\r\n"
                    "User-Agent: Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)\r\n"
                    "Connection: close\r\n"
                    "\r\n";

		CNetAddr *pSeeds = new CNetAddr[100];
		int seedCount = 0;
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. ApeCoin is probably already running."), addrBind.ToString().c_str());
        else
